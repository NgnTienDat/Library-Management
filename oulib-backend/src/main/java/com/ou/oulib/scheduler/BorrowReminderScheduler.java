...
import com.ou.oulib.infras.producer.EmailReminderPublisher;
...
public class BorrowReminderScheduler {
    ...
    EmailReminderPublisher emailReminderPublisher;
    ...
    // .findByStatusAndReturnDateIsNullAndReminderSentFalseAndDueDate(BorrowStatus.BORROWING,
    // dueDate);
    //
    // log.info("Found {} borrow records due tomorrow ({})",
    // recordsDueTomorrow.size(), dueDate);
    //
    ...
    // } catch (Exception e) {
    // log.error("Failed to publish reminder for borrow record: {}", record.getId(),
    // e);
    // }
    // }
    //
    ...
    public void sendBorrowReminders() {
        ...
        for (BorrowRecord record : recordsDueToday) {
            try {
                // borrowReminderProducer.publishBorrowReminder(record.getId());
                emailReminderPublisher.publishBorrowEmailReminder(record.getId());
                record.setReminderSent(true);
                borrowRecordRepository.save(record);
                log.info("Published reminder for borrow record: {}", record.getId());
            } catch (Exception e) {
...
// /**
// * Runs every minute.
// * Queries borrow records that are due within 3 minutes and publishes reminder
// messages.
// */
...
//
// log.info("Found {} borrow records due within 3 minutes ({})",
// recordsDueSoon.size(), reminderTime);
//
